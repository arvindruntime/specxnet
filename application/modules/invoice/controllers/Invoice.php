...
class Invoice extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library(array('Invoice_library' => 'Invoice'));
...

	{
		...
		$this->page->setFooterJs(base_url() . "assets/custom/js/invoice.js");
		...
		// $page['parentCompany'] = $this->InvoiceModel->getCompany($select2,$where);
		...
		// $page['country'] = $this->InvoiceModel->getCountry($select);
		...
		$page['getSalesperson'] = $this->InvoiceModel->getSalesperson($select, $where);
		...
		$page['getSavedFilter'] = $this->InvoiceModel->getSavedFilter($select2, $where2);



		$page['config'] = $this->Invoice->getConfig();

		$page['tabs'] = $this->Invoice->getTabs();

		$page['action'] = $this->Invoice->getAction();

		$page['proposalColumn'] = $this->Invoice->getProposalColumn();

		$page['checkedAction'] = $this->Invoice->getCheckedAction();

		$page['module_name'] = $this->uri->segment(1);

		$page['gridView'] = $this->InvoiceModel->getGrid('*', array('module_name' => $page['module_name']));
	...
	public function addProposal()
	{
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$getCompanyLeadOpp = $this->InvoiceModel->getCompanyLeadOpp($select, $where);



				$select = "count(title) AS totalcount";

				$getCountOfExistingLead = $this->InvoiceModel->getCountOfExistingLead($select, $getCompanyLeadOpp[0]['name']);
				...
				if (!isset($post['p_id'])) {
					...
					$data['id'] = $this->InvoiceModel->insertProposal($insertArray);
					...
					$data['itemId'] = $this->InvoiceModel->insertIdProposalWorksheet($add_P_id);
					...
					$data['id'] = $this->InvoiceModel->updateCompany($insertArray, $where);
	...
	public function getLeadCompany()
	{
		...
		try {
			...
			$getCompanyLeadOpp = $this->InvoiceModel->getCompanyLeadOpp($select, $where, $autocomplete);
	...
	public function displayForm($id = null, $pw_id = null)
	{
		...
		try {
			...
			if ($Permission) {
				...
				$data['leadOpp'] = $this->InvoiceModel->getLeadOpportunities($select, $where);
				...
				$data['supplier'] = $this->InvoiceModel->getSupplierList($select2, $where2);
				...
				if ($id) {
					...
					$data['value'] = $this->InvoiceModel->getProposallist($select, $where, $join);
					// print_r($data['value']);
					...
					$getItemList = $this->InvoiceModel->getItemList($select2, $where2);
					...
					$data['getFormat'] = $this->InvoiceModel->getFormat($select2, $where2);
					...
					$data['getFobCif'] = $this->InvoiceModel->getTotalFOBCIF($select2, $where2);
				...
				// 	$data['getFormat'] = $this->InvoiceModel->getFormat($select3, $where3);
	...
	public function getComment()
	{
		try {
			...
			$data['comment'] = $this->InvoiceModel->getComment($select, $where);
	...
	public function addComment()
	{
		try {
			...
			$data['comment'] = $this->InvoiceModel->addComment($insertCommentArray);
			...
			$getLeadOpportunityID = $this->InvoiceModel->getLeadOpportunityID($wherePid);
			...
			$getLeadOpportunityID = $this->InvoiceModel->convertLead($updateLeadArray, $whereLid);



			$data = $this->InvoiceModel->setStatus($updateArray, $p_id);
			...
			// 	);
			// 	$data['comment'] = $this->InvoiceModel->addInvoice($insertInvoiceArray);
			// }
			// $html = $this->page->getPage('proposalComment', $data, true);
		...
		try {
			...
			if ($Permission) {
				...
				if ($id) {
					...
					// $dataPO = $this->InvoiceModel->getPO($wherecond);
	...
	public function getItem($var = null)
	{
		...
		$data['value'] = $this->InvoiceModel->getProposallistdemo($select);
	...
	public function getInvoice()
	{
		...
		if (!empty($request)) {
			...
			if (array_key_exists("q", $request) && !empty(json_decode($request['q']))) {
				...
				if (!empty($request['search']['value'])) {
					...
					$where = array('i.customer_id' => $this->session->userdata('user_id'));
				}
			}
		} else {
			...
			if ($user_type == 'Customer') {
				...
				$where = array('fk_lead_opportunity_id' => $lead_opportunity_id, 'u.user_id' => $this->session->userdata('user_id'), 'i.customer_id' => $this->session->userdata('user_id'));

				$where2 = array('fk_lead_opportunity_id' => $lead_opportunity_id);
			} else {

				$where = array('fk_lead_opportunity_id' => $lead_opportunity_id);
		...
		$proposalList = $this->InvoiceModel->getInvoicelist($select, $where, $order, $limit, $offset);

		foreach ($proposalList as $key => $value) {

			$proposalList[$key]['title']=str_replace("Proposal", "Invoice", $proposalList[$key]['title']);
		...
		if (!empty($request)) {
			...
			if ($user_type == 'Customer') {
				...
				$companycount = $this->InvoiceModel->getProposallistCount('count(*) as count', $where);
			}
		} else {
			...
			if ($user_type == 'Customer') {
				...
				$companycount = $this->InvoiceModel->getProposallistCount('count(*) as count', $where2);
			}
		}
	...
	public function getCompanyIdentifier($country)
	{
		...
		$companyIdentifierList['list'] = $this->InvoiceModel->getCompanyIdentifier($where);
	...
	public function getItemList()
	{
		...
		try {
			...
			$data = $this->InvoiceModel->getItemList($select2, $where2);
			$propsalworksheet = "";
	...
	function getFilter()
	{
		...
		$getFilterData['data'] = $this->InvoiceModel->getFilterById($postData['filter_id']);
	...
	public function addFilter()
	{
		...
		try {
			...
			$AddCompany = $this->InvoiceModel->saveFilter($postData);
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (isset($post['pw_id']) && $post['pw_id'] != '') {
					...
					$data['id'] = $this->InvoiceModel->insertProposalWorksheet($post, $pw_id);
					...
					$data['id'] = $this->InvoiceModel->insertProposalWorksheet($post);
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (isset($post['fk_pid']) && $post['fk_pid'] == '') {
					...
					$data['id'] = $this->InvoiceModel->insertProposalFormat($insertArray, $post['pf_id']);
					...
					$data['id'] = $this->InvoiceModel->insertProposalFormat($insertArray);
...

	{
		...
		try {
			...
			foreach ($rows as $key => $value) {
				...
				$importExcelData = $this->InvoiceModel->insertProposalWorksheet($insertArray);
	...
	public function createExcel()
	{
		...
		if (isset($selectedColumns['columnFields'])) {
			...
			$proposalData = $this->InvoiceModel->exportCompany($columnString, $proposalList);
	...
	public function createGrid()
	{
		...
		try {
			...
			$column = $arrayField = $this->Invoice->getProposalColumn();
			...
			if (!empty($selectedColumns) && $gridId == 'null') {
				...
				$grid_columns = $this->InvoiceModel->getGrid('grid_columns', array('grid_id' => $gridId));
			...
			if (isset($selectedColumns['ischeck']) && $selectedColumns['ischeck'] != '' && $selectedColumns['ischeck'] != 'undefined') {
				...
				$this->InvoiceModel->insertGrid($columns);
			}

			$data['columns'] = $arrayField;

			$data['options'] = $this->InvoiceModel->getGrid('*', array('module_name' => $module_name));
	...
	public function deleteproposal()
	{
		...
		$DeletedUser = $this->InvoiceModel->deleteproposal($p_id);
	...
	public function getEmailIds()
	{
		...
		$getData = $this->InvoiceModel->getEmailIds($select, $newUsrId);
	...
	public function saveProposal()
	{
		...
		$updateProposal = $this->InvoiceModel->updateProposal($updateArray, $where);
	...
	public function get_InvoicePreview($val)
	{
	// exit();	
		try {
			// $where2 = .....array('pw.fk_pid' => $val);
			$data['formheading'] = 'Invoice Order Preview';
	
			// $select = "*";
			// $where = array('p_id' => $val);
			// $getCompanyLeadOppId = $this->InvoiceModel->getProposal($select, $where);
			// $data['proposal']=$getCompanyLeadOppId[0];
			$select = "i.*,po.*";
			$where = array('i.fk_pid' => $val);
			$getInvoice = $this->InvoiceModel->getInvoicelist($select, $where);
			$data['invoice']=$getInvoice[0];
			...
			// $getSalesperson = $this->InvoiceModel->getUsersDetails($where1,$select1);
			// $data['getSalesperson'] = $getSalesperson[0];
	
			// $select2 = "u.full_name,uc.contact_type,uc.contact_info,c.*";
			// $where2 = array('user_id' => $admin_id);
			// $admin = $this->InvoiceModel->getUsersDetails($where2,$select2);
			// $data['admin'] = $admin[0];
			...
			$html = $this->page->getPage('getInvoicePreview', $data, true);
...

	{
		...
		$this->InvoiceModel->insertPO($insertData);
	}
...

	{
		...
		$data = $this->InvoiceModel->getPO($where);
		...
		if (!empty($data)) {
			...
			$this->InvoiceModel->insertInvoice($insertData);
		}
	}
}
