...
class Proposal extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library(array('Proposal_library' => 'proposal'));
		$this->load->model(array('Invoice_model' => 'InvoiceModel'));
	...
	public function index()
	{
		...
		// set title
		$this->page->setTitle('Lead Proposal');
		// set head style
		...
		$this->page->setFooterJs(base_url() . "assets/custom/js/proposal.js");
		...
		$page['getSalesperson'] = $this->proposalModel->getSalesperson($select, $where);
		// -- Get Saved Filter
		$select2 = "filter_id, filter_name, filter_values";
		$where2 = array('module' => 'Proposal');
		$page['getSavedFilter'] = $this->proposalModel->getSavedFilter($select2, $where2);
		$page['config'] = $this->proposal->getConfig();
		$page['tabs'] = $this->proposal->getTabs();
		$page['action'] = $this->proposal->getAction();
		$page['proposalColumn'] = $this->proposal->getProposalColumn();
		$page['checkedAction'] = $this->proposal->getCheckedAction();
		$page['module_name'] = $this->uri->segment(1);
		$page['gridView'] = $this->proposalModel->getGrid('*', array('module_name' => $page['module_name']));
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (!isset($post['p_id'])) {
					...
					$data['id'] = $this->proposalModel->updateCompany($insertArray, $where);
	...
	public function getLeadCompany()
	{
		try {
			...
			$autocomplete = true;
			$getCompanyLeadOpp = $this->proposalModel->getCompanyLeadOpp($select, $where, $autocomplete);
	...
	public function displayForm($id = null, $pw_id = null)
	{
		try {
			...
			if ($Permission) {
				...
				$data['leadOpp'] = $this->proposalModel->getLeadOpportunities($select, $where);
				$select2 = 'company_id, company_name';
				$where2 = array('company_type' => 'Supplier', 'activity_status' => 'active');
				$data['supplier'] = $this->proposalModel->getSupplierList($select2, $where2);
				$data['formheading'] = 'Proposal Details';
				$data['value'] = array();
				if ($id) {
					...
					$join = true;
					$data['value'] = $this->proposalModel->getProposallist($select, $where, $join);
					// print_r($data['value']);
					...
					$getItemList = $this->proposalModel->getItemList($select2, $where2);
					foreach ($getItemList as $Wproposal) {
						...
						$fk_pw_id[] = $Wproposal['pw_id'];
						$propsalworksheet[] = $Wproposal;
					}
					$data['getItemList'] = $propsalworksheet;
					$data['fk_pw_id'] = implode(',', $fk_pw_id);
					$select2 = "pf.*";
					$where2 = array('pf.fk_p_id' => $id);
					$data['getFormat'] = $this->proposalModel->getFormat($select2, $where2);
					$select2 = "sum(total_price_fob) as fob, sum(total_price_cif) as cif";
					$where2 = array('fk_pid' => $id);
					$data['getFobCif'] = $this->proposalModel->getTotalFOBCIF($select2, $where2);
					$approvePermission = $this->permission->checkUserPermission(35);
					// echo $approvePermission;
					// echo $this->session->userdata('access');
					// exit();
					if ($approvePermission && $this->session->userdata('adminAccess') == 'No' && $data['value']['status'] != 'Declined' && $data['value']['status'] != 'Released PO') {
					...
					} else if (($data['value']['status'] != 'Released' && $data['value']['status'] != 'Declined' && $data['value']['status'] != 'Released PO' && $data['value']['status'] != 'Released Invoice') && $this->session->userdata('adminAccess') == 'Yes') {
					...
					} else if ($user_type == 'Internal User' && $this->session->userdata('adminAccess') == "Yes" && $data['value']['status'] == 'Released PO') {
				...
				$data['p_id'] = $id;
				$select3 = "po.*";
				$where3 = array('po.fk_pid' => $id);
				$getPO = $this->POModel->getPOlist($select3, $where3);
				if (!empty($getPO[0])) {
					// print_r($getPO[0]['po_id']);
					$data['po_id'] = $getPO[0]['po_id'];
				}
	...
	public function getComment()
	{
		try {
			...
			$data['comment'] = $this->proposalModel->getComment($select, $where);
	...
	public function addComment()
	{
		$post = $this->input->post();
			...
			//$data['comment'] = $this->proposalModel->addComment($insertCommentArray);
			$user_id = $this->session->userdata('user_id');
			$user_type = $this->session->userdata('user_type');
			...
			$getLeadOpportunityID = $this->proposalModel->getLeadOpportunityID($wherePid);
			...
			$getLeadOpportunityID = $this->proposalModel->convertLead($updateLeadArray, $whereLid);

			$data = $this->proposalModel->setStatus($updateArray, $p_id);
			$select = "fk_b_id";
			$wherePid = array('p_id' => $p_id);
			$getProposal = $this->proposalModel->getProposallist($select,$wherePid);
			$update = $this->rfqModel->updateRFQ(['status' => 'Accepted'], ['b_id' => $getProposal[0]['fk_b_id']]);
			if($status=='Declined'){
				$select = "fk_b_id";
				$wherePid = array('p_id' => $p_id);
				$getProposal = $this->proposalModel->getProposallist($select,$wherePid);
				// print_r($getProposal[0]['fk_b_id']);exit();
				if($getProposal[0]['fk_b_id']!='0'){
					$data = $this->rfqModel->setStatus($updateArray, $getProposal[0]['fk_b_id']);
				}
			}
			// exit();
			// $html = $this->page->getPage('proposalComment', $data, true);
	...
	public function addReleaseStatus()
	{
		try {
			$post = $this->input->post();
			$status = $post['status'];
			$p_id = $post['fk_pid'];
			$updateProposalArray = array(
				'proposal_no' => $post['proposal_no'],
			);
			$where = array('p_id' => $post['fk_pid']);
			$updateProposal = $this->proposalModel->updateProposal($updateProposalArray, $where);
			$user_id = $this->session->userdata('user_id');
			$user_type = $this->session->userdata('user_type');
			$updateArray = array(
				'status' => $status,
	...
	public function addPO()
	{
		try {
			$post = $this->input->post();
			// print_r($post);exit();
			$select = "approved_by_admin,approved_by_customer,fk_lead_opportunity_id";
			$where = array('p_id' => $post['fk_pid']);
			...
			$select1 = "u.full_name,uc.contact_type,uc.contact_info";
			...
			$select2 = "u.full_name,uc.contact_type,uc.contact_info,c.company_name,c.company_logo_file,c.street_address,c.city,c.state,c.country,c.zip_code,c.dialing_code,c.bussiness_contact";
			$where2 = array('u.user_id' => $admin_id, 'uc.contact_type' => 'Email');
			$admin = $this->POModel->getUsersDetails($where2, $select2);
			$data['admin'] = $admin[0];

			$CompanyLeadOppId = $getCompanyLeadOppId[0]['fk_lead_opportunity_id'];
			$select = "c.company_name,c.company_logo_file,c.street_address,c.city,c.state,c.country,c.zip_code,c.dialing_code,c.bussiness_contact";
			$where = array('lo.lead_opportunity_id' => $CompanyLeadOppId);
			$getCompanyLeadOpp = $this->POModel->getCompanyLeadOpp($select, $where);
			$data['companyDetails'] = $getCompanyLeadOpp[0];

			$cc_email = "";
			$vc_email = "";
			if ($data['getSalesperson']['contact_type'] == "Email") {
				$cc_email = $data['getSalesperson']['contact_info'];
			}
			if ($data['admin']['contact_type'] == "Email") {
				$vc_email = $data['admin']['contact_info'];
			}
			// $PO_no = "S-" . time() . $this->generate_invoice(6);
			$PO_no = $post['po_no'];
			...
			$insertPOArray = array(
				...
				'status' => $post['status'],
				'po_no' => $PO_no,
				'created_by_customer' => $this->session->userdata('user_id'),
				'fk_pw_id' => $post['fk_pwid'],
				'cc_name' => $data['companyDetails']['company_name'],
				'cc_logo' => $data['companyDetails']['company_logo_file'],
				'cc_address' =>  $data['companyDetails']['street_address'] . ", " . $data['companyDetails']['city'] . ", " . $data['companyDetails']['state'] . ", " . $data['companyDetails']['country'] . ", " . $data['companyDetails']['zip_code'],
				'cc_business_contact' => $data['companyDetails']['dialing_code'] . " " . $data['companyDetails']['bussiness_contact'],
				'cc_contact_person' => $data['getSalesperson']['full_name'],
				'cc_email' => $cc_email,
				'vc_name' => $data['admin']['company_name'],
				'vc_logo' => $data['admin']['company_logo_file'],
				'vc_address' =>  $data['admin']['street_address'] . ", " . $data['admin']['city'] . ", " . $data['admin']['state'] . ", " . $data['admin']['country'] . ", " . $data['admin']['zip_code'],
				'vc_business_contact' => $data['admin']['dialing_code'] . " " . $data['admin']['bussiness_contact'],
				'vc_contact_person' => $data['admin']['full_name'],
				'vc_email' => $vc_email,
				'customer_id' => $post['customer_id'],
			);
			// print_r($insertPOArray);exit();
			$this->proposalModel->addPO($insertPOArray);
			$user_id = $this->session->userdata('user_id');
			$user_type = $this->session->userdata('user_type');
	...
	function generate_invoice($l, $c = '1234567890')
	{
	...
	public function addInvoice()
	{
		try {
			$post = $this->input->post();
			// print_r($post);exit();
			// $invoice_no = "S-" . date('y') . date('d') . $this->generate_invoice(6);
			$invoice_no = $post['invoice_no'];
			$insertInvoiceArray = array(
				'fk_pid' => $post['fk_pid'],
				'fk_po_id' => $post['fk_po_id'],
				'status' => $post['status'],
				'invoice_no' => $invoice_no,
				'created_by_customer' => $this->session->userdata('user_id'),
				'customer_id' => $post['customer_id']
			);
			// print_r($insertInvoiceArray);exit();
			$this->proposalModel->addInvoice($insertInvoiceArray);
			$user_id = $this->session->userdata('user_id');
			$user_type = $this->session->userdata('user_type');
			$post = $this->input->post();
			$status = $post['status'];
	...
	{
		// getInvoicelist
		$where = array('i.invoice_id' => $invoice_id);
		$select = "i.invoice_id";
		$proposalList = $this->InvoiceModel->getInvoicelist($select, $where);
		// print_r($proposalList);
		...
		try {
			...
			if ($Permission) {
				...
				if ($id) {
					...
					$where2 = array('fk_pid' => $id);
					$data['getFobCif'] = $this->proposalModel->getTotalFOBCIF($select2, $where2);
					if ($user_type == 'Customer' && $this->session->userdata('adminAccess') == 'No') {
					...
					// $dataPO = $this->proposalModel->getPO($wherecond);
					if ($user_type == 'Customer' && ($this->session->userdata('adminAccess') == 'No')) {
	...
	public function getItem($var = null)
	{
		...
		$data['value'] = $this->proposalModel->getProposallistdemo($select);
		if (is_array($data['value'])) {
			$data['value'] = $data['value'][0];
		}
	...
	public function getProposals()
	{
		...
		if (!empty($request)) {
			...
			if (array_key_exists("q", $request) && !empty(json_decode($request['q']))) {
				...
				if (isset($requestNew->approval_deadline) && !empty($requestNew->approval_deadline)) {
					if ($requestNew->approval_deadline != '') {
						$newDate = date("d-m-Y", strtotime($requestNew->approval_deadline));
					} else {
						$newDate = $requestNew->approval_deadline;
					}
					$where['p.approval_deadline'] = $newDate;
					$data['approval_deadline'] = $newDate;
				}
				...
				if (!empty($request['search']['value'])) {
					...
					$where = array('u.user_id' => $this->session->userdata('user_id'), 'p.status !=' => 'Drafted');
				}
			}
		} else {
			$post = $this->input->post();
			...
			if ($user_type == 'Customer') {
				$where = array('fk_lead_opportunity_id' => $lead_opportunity_id, 'p.status !' => 'Drafted', 'u.user_id' => $this->session->userdata('user_id'));
				$where2 = array('fk_lead_opportunity_id' => $lead_opportunity_id, 'status !' => 'Drafted');
			} else {
		...
		$order = null;
		$select = "p.*, lo.lead_opportunity_id, lo.opportunity_title, c.company_name, CONCAT(u.first_name, ' ', u.last_name) AS sales_people, CONCAT('$', SUM(pw.total_price_ex_factory)) as total_price_ex_factory, CONCAT('$', SUM(pw.unit_total_price_fabric)) as total_price_fabric, CONCAT('$', SUM(pw.unit_total_price_leather)) as total_price_leather, CONCAT('$', SUM(pw.total_price_fob)) as total_fob, CONCAT('$', SUM(pw.total_price_cif)) as total_cif";

		if (isset($request['order']) && is_array($request['order'])) {
			$order = $columns[$request['order'][0]['column']] . ' ' . $request['order'][0]['dir'];
		}
		$proposalList = $this->proposalModel->getProposallist($select, $where, $order, $limit, $offset);
		foreach ($proposalList as $key => $value) {
			// echo $proposalList[$key]['p_id'];
			if ($proposalList[$key]['approval_deadline'] == "") {
				$proposalList[$key]['approval_deadline'] = "---";
				//$opportunityDataList[$key]['age'] = "---";
			} else {
				$leadactiDate = $proposalList[$key]['approval_deadline'];
				$actvity_up_date = date("d-M-Y", strtotime($leadactiDate));
				$proposalList[$key]['approval_deadline'] = $actvity_up_date;
			}
		}
		...
		if (!empty($request)) {
			if ($user_type == 'Customer') {
				...
				$companycount = $this->proposalModel->getProposallistCount('count(*) as count', $where);
			}
		} else {
			if ($user_type == 'Customer') {
				$companycount[0]['count'] = count($proposalList);
			} else {
				$companycount = $this->proposalModel->getProposallistCount('count(*) as count', $where2);
			}
		}
	...
	public function getCompanyIdentifier($country)
	{
		...
		$companyIdentifierList['list'] = $this->proposalModel->getCompanyIdentifier($where);
	...
	public function getItemList()
	{
		...
		try {
			...
			$data = $this->proposalModel->getItemList($select2, $where2);
			$propsalworksheet = array();
	...
	function getFilter()
	{
		...
		$getFilterData['data'] = $this->proposalModel->getFilterById($postData['filter_id']);
		$select = "u.user_id, CONCAT (u.first_name,' ',u.last_name) AS name";
		$where = array('u.active_status' => 'active');
		$page['getSalesperson'] = $this->proposalModel->getSalesperson($select, $where);
	...
	public function addFilter()
	{
		...
		try {
			...
			$AddCompany = $this->proposalModel->saveFilter($postData);
			$response['code'] = 200;
			$response['message'] = "<div class='alert alert-success'>
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (isset($post['pw_id']) && $post['pw_id'] != '') {
					...
					$data['id'] = $this->proposalModel->insertProposalWorksheet($post);
					$response['code'] = 200;
		...
		try {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (isset($post['fk_pid']) && $post['fk_pid'] == '') {
					$data['id'] = $this->proposalModel->insertProposalFormat($insertArray, $post['pf_id']);
					$response['message'] = "<div class='alert alert-success'>
	                    <strong>Success!</strong> Proposal Format Updated Successfully.</div>";
				} else {
					$data['id'] = $this->proposalModel->insertProposalFormat($insertArray);
					$response['message'] = "<div class='alert alert-success'>
	...
	public function importData($proposal_id = null)
	{
		...
		try {
			...
			foreach ($rows as $key => $value) {
				...
				$importExcelData = $this->proposalModel->insertProposalWorksheet($insertArray);
	...
	public function createExcel()
	{
		...
		if (isset($selectedColumns['columnFields'])) {
			...
			$proposalData = $this->proposalModel->exportCompany($columnString, $proposalList);
			$alphabets = range('A', 'Z');
	...
	public function createGrid()
	{
		try {
			...
			$column = $arrayField = $this->proposal->getProposalColumn();
			$data['columns'] = $column;
			...
			if (!empty($selectedColumns) && $gridId == 'null') {
				...
				$grid_columns = $this->proposalModel->getGrid('grid_columns', array('grid_id' => $gridId));
			...
			if (isset($selectedColumns['ischeck']) && $selectedColumns['ischeck'] != '' && $selectedColumns['ischeck'] != 'undefined') {
				...
				$this->proposalModel->insertGrid($columns);
			}
			$data['columns'] = $arrayField;
			$data['options'] = $this->proposalModel->getGrid('*', array('module_name' => $module_name));
			$response['code'] = 200;
	...
	public function deleteproposal()
	{
		...
		$DeletedUser = $this->proposalModel->deleteproposal($p_id);
		if ($DeletedUser) {
	...
	public function getEmailIds()
	{
		...
		$getData = $this->proposalModel->getEmailIds($select, $newUsrId);
		$test = [];
	...
	public function saveProposal()
	{
		...
		$updateProposal = $this->proposalModel->updateProposal($updateArray, $where);
	...
	public function get_preview($val)
	{
		...
		$data['getFormat'] = $this->proposalModel->getFormat($select3, $where3);
		// print_r($data['getFormat']);
		$getData = $this->load->view('pages/get_proposalpreview', $data);
		return $getData;
	}
	...
	public function releasePO()
	{
		...
		$this->proposalModel->insertPO($insertData);
	}
