...
class ActivityCronjob extends CI_Controller {

	public function saveDraft()
	{
		//---- Activity Module Starts----------------------------
		$this->load->model(array('Activity_model' => 'leadActivityModel'));

		$select1 = 'de.*, uc.*, up.*';
		$where1 = array('uc.contact_type' => 'Email');
		$userData = $this->leadActivityModel->getDraftEmails($select1, $where1);
	...
	public function reminder()
	{
		...
		$select1 = 'activity_title, reminder, email_draft_time';
		$where1 = array('activity_status' => 'active');
		$userData = $this->leadActivityModel->getReminders($select1, $where1);
		//print_r($userData['result']);exit;
		if ($userData['count'] != 0) {
			foreach ($userData['result'] as $key => $value) {
				$draft_time = strtotime($value['email_draft_time']);
				// $workman_certificate_expires_date1 = strtotime($value['workman_certificate_expires']);
				if(strpos($value['reminder'], 'min') !== false){
				    $mod_type = 'users';
				} else{
				    $mod_type = 'company';
				}
				...
				date_default_timezone_set('Asia/Kolkata');
				$setdraft_time = $this->getDateDiff($draft_time, strtotime(date('Y-m-d h:i A')), $mod_type);

				if ($setdraft_time == $matches) {
					$to = $value['email_Ids'];
					$from = $value['emails'];
					$password = $this->my_simple_crypt($value['password'], 'd' );
					$subject = $value['subject'];
					$mail_message = '<html>
                            ...
                            <body>
                            	...
                            	You have to contact the activity <b>'.$value['activity_title'].'</b> Today.
                            	<br/>
